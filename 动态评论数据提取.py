...
def GetUsers():
    ...
    has_more = 1
    offset = ''
    DynamicAPI = "https://api.bilibili.com/x/v2/reply?jsonp=jsonp&pn={}&type=11&oid={}&sort=1"
    conn = sqlite3.connect('Bilibili_TMP.db')
    c = conn.cursor()
    loopcount = Total_count /20
    while Tmp_count<loopcount:
        Tmp_DynamicAPI = DynamicAPI.format(Tmp_count,Dynamic_id)
        try:
            # print("BiliJsonbegin",Tmp_DynamicAPI) 
            BiliJson = json.loads(GetMiddleStr(urllib2.urlopen(Tmp_DynamicAPI).read(),"replies\":",",\"hots"))
            for BiliJson_dict in BiliJson:
                Bilibili_UID = str(BiliJson_dict['member']['mid'])
                Bilibili_Uname = BiliJson_dict['member']['uname']
                Bilibili_Comment = BiliJson_dict['content']['message']
                Bilibili_Ts = BiliJson_dict['ctime']
...
def GetTotalRepost():
    global Total_count
    DynamicAPI = "https://api.bilibili.com/x/v2/reply?jsonp=jsonp&pn=1&type=11&oid={}&sort=1".format(Dynamic_id)
    BiliJson = json.loads(urllib2.urlopen(DynamicAPI).read())
    Total_count = BiliJson['data']['page']['count']
...
def PrintAll():
    ...
    for row in cursor:
        ...
        print("第", i,"个评论 用户ID:", row[0]," 用户名:", row[1],"评论详情：", row[2], "转发时间：", dt) 
    print("总数:",Total_count) 
    print("抽奖uid:",UP_UID) 
...
def WriteIntoText():
    ...
    textname = '评论记录' + str(time.strftime('%Y%m%d %H%M%S',time.localtime(time.time())))
    ...
    for row in cursor:
        ...
        # 转换成新的时间格式
        dt = time.strftime("%Y-%m-%d %H:%M:%S", time_local)
        strline = "第"+ str(i)+"个评论 用户ID:"+ str(row[0])+" 用户名:"+ row[1]+" 评论详情："+ row[2]+ " 转发时间："+ dt + '\n'
        f.write(strline)
    f.close
...
def GetDynamicid():
    ...
    try:
        ...
        if bilibili_domain == "t.bilibili.com":
            ...
            bilibili_docid = "https://api.vc.bilibili.com/dynamic_svr/v1/dynamic_svr/get_dynamic_detail?dynamic_id=" + str(nums[0])
            #  Dynamic_id = GetMiddleStr(urllib2.urlopen(bilibili_docid).read(),'rid\":',',\"acl')
            dynamicjson = json.loads(urllib2.urlopen(bilibili_docid).read())
            userinfo = dynamicjson['data']['card']['desc']['user_profile']['info']
            Dynamic_id = dynamicjson['data']['card']['desc']['rid']
            UP_UID = userinfo['uid']
            UP_NAME = userinfo['uname'] 
    ...
    #        return str(Dynamic_id)
       ...
       print(TellTime() + "您输入的网址有误！", str(reason)) 
       exit()
def TellTime():
